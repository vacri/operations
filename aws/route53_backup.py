#!/usr/bin/env python3
# modified version of https://github.com/jacobfgrant/route53-lambda-backup3
# * remove bucket creation
# * change destination storage patterns
#    * use a prefix inside our general backup bucket
#    * use s3-native 'previous versions' rather than timestamps in filenames
#      as the method for 'older backups'
...
def upload_to_s3(folder, filename, bucket_name, key):
    ...
    key = folder + '/' + key
    #print("...uploading %s to %s/%s" % (filename, bucket_name, key))
...
def lambda_handler(event, context):
    ...
    #bucket_response = create_s3_bucket(s3_bucket_name, s3_bucket_region)
        ...
        #return False
    print("Fetching list of hosted zones...")
    hosted_zones = get_route53_hosted_zones()
    for zone in hosted_zones:
        print("...%s" % zone['Name'][:-1])
        zone_folder = (s3_path + '/' + zone['Name'][:-1])
